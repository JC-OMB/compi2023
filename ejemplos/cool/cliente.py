...
class TypeChecker(coolListener):
    ...
    def exitEq(self, ctx:coolParser.EqContext):
        #bad equality, compara tipos y si no son iguales manda error
        if ctx.expr(0).type != ctx.expr(1).type:
            raise Exception("bad equality")
    ...
    def exitCallobj(self, ctx:coolParser.CallobjContext):
        #raises exception para baddispatch y whilebody
        try:
            ctx.expr(0).type.lookupMethod(ctx.ID().getText())
        except:
            raise Exception("Método {} no encontrado en la clase {}".format(1, 2))
...
if __name__ == '__main__':
    ...
    main('../../resources/semantic/input/badequalitytest.cool')
    #main('../../resources/semantic/input/badwhilebody.cool')
